...
1. Click "[Use this template](https://github.com/cipscis/activate/generate)" to create a new repository based on this one.
2. Update names throughout the package.
	a. Replace `activate` with the name of your package as it is used in code.
	b. Replace `Activate` Replace with the name of your package as it is used in documentation.
	c. Rename `src/activate.ts` and replace `activate` with the name of your main entry point file.
...
In the `docs` folder, which can be deployed to GitHub Pages but is not included when your package is installed, you can document your package. Webpack is configured to have an alias for your main entry point so you can load it as though it were installed from npm, e.g. `import { foo } from '@cipscis/activate';`
...
Then, you can run `npm publish` to publish your package. Once published, you can run `npm install @cipscis/activate` to install the package in other projects.

## Structure

### Frontend assets

By default, your package consists of the contents of the `dist` folder. This folder is populated when the contents of the `src` folder are compiled using `tsc`, which happens automatically prior to publishing. The `src` folder contains a single TypeScript file called `activate.ts`. You can rename this file, but if you do make sure you update the [`browser`](https://docs.npmjs.com/cli/v7/configuring-npm/package-json#browser) property in your `package.json` file. If your package doesn't need to be run in a browser, you should change this property to [`main`](https://docs.npmjs.com/cli/v7/configuring-npm/package-json#main).
...
When publishing a project using [GitHub Pages](https://pages.github.com/), the project usually appears at a URL with a path, such as `https://cipscis.github.io/activate`. This means using root relative URLs such as `/assets/css/main.css` would work locally, but would break when the project is published on GitHub Pages.
...
![Build and deploy status badge](https://github.com/cipscis/activate/actions/workflows/build-and-deploy.yml/badge.svg)
...
See [Activate documentation](https://cipscis.github.io/activate/)
